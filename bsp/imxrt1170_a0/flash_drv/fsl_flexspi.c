...
status_t flexspi_configure_dll(uint32_t instance, flexspi_mem_config_t *config)
{
    ...
    bool mdisConfigRequired = false;

    do
    {
        FLEXSPI_Type *base = flexspi_get_module_base(instance);
        bool isUnifiedConfig = true;
        uint32_t flexspiRootClk = 0u;
        uint32_t flexspiDll[2] = { 0 };
        uint32_t dllValue = 0u;
        uint32_t temp = 0u;
        ...
        else
        {
            ...
            if (useDLL)
            {
                flexspiDll[0] = FLEXSPI_DLLCR_DLLEN(1) | FLEXSPI_DLLCR_SLVDLYTARGET(0x0F) | (0x4 << 20);
                flexspiDll[1] = FLEXSPI_DLLCR_DLLEN(1) | FLEXSPI_DLLCR_SLVDLYTARGET(0x0F) | (0x4 << 20);
            }
...
status_t flexspi_config_flash_control_registers(uint32_t instance, flexspi_mem_config_t *config)
{
    ...
    do
    {
        ...
        uint32_t temp = 0u;
        uint32_t serialClockFrequency = 0u;
        uint32_t csIntervalTicks = 0u;
        ...
        if (config->controllerMiscOption & FLEXSPI_BITMASK(kFlexSpiMiscOffset_WriteMaskEnable))
        {
            if ((config->sflashA1Size > 0) || (config->sflashA2Size > 0))
            {
                base->FLSHCR4 |= FLEXSPI_FLSHCR4_WMENA_MASK;
            }
            if ((config->sflashB1Size > 0) || (config->sflashB2Size > 0))
            {
                base->FLSHCR4 |= FLEXSPI_FLSHCR4_WMENB_MASK;
            }
        }

        if (config->controllerMiscOption & FLEXSPI_BITMASK(kFlexSpiMiscOffset_WriteOpt1Clear))
        {
            base->FLSHCR4 &= (uint32_t)~FLEXSPI_FLSHCR4_WMOPT1_MASK;
        }
        else
        {
...
status_t flexspi_device_cmd_config(uint32_t instance, flexspi_mem_config_t *config, uint32_t baseAddr)
{
    ...
    do
    {
        ...
        if (index >= 3) // It is highly recomended to use a constant/macro to replace the directly used number 3
        {
            status = kStatus_Success;
            break;
        }
...
//!@brief Update DLLCR
void flexspi_set_dllcr(uint32_t instance, uint32_t value)
{
    do
    {
        FLEXSPI_Type *base = flexspi_get_module_base(instance);
        ...
        base->DLLCR[0] = FLEXSPI_DLLCR_OVRDEN(1) | FLEXSPI_DLLCR_OVRDVAL(value);

    } while (0);
}

//!@brief Update Read Sampling Clock source
void flexspi_update_read_clock_source(uint32_t instance, uint32_t src)
{
    do
    {
        FLEXSPI_Type *base = flexspi_get_module_base(instance);
        ...
        base->MCR0 = (base->MCR0 & ~FLEXSPI_MCR0_RXCLKSRC_MASK) | FLEXSPI_MCR0_RXCLKSRC(src);
